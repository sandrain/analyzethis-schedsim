...
function report_mean {
	scheduler=$1
	workflow=$2
	echo "mean utilization, $workflow : $scheduler"
	...
	for n in `seq 1 6`; do
		...
		for ntask in `seq 30 10 100`; do
			...
			runtime="`cat $file | grep '^OSD mean' | awk {'print $5'}`"
...
function report_std {
	scheduler=$1
	workflow=$2
	echo "std utilization, $workflow : $scheduler"
	...
	for n in `seq 1 6`; do
		...
		for ntask in `seq 30 10 100`; do
			file="${nosd}_${scheduler}_${workflow}_${ntask}.txt"
			runtime="`cat $file | grep '^OSD std' | awk {'print $5'}`"
			echo -e "$nosd\t$ntask\t$runtime"
		done
	done
}
